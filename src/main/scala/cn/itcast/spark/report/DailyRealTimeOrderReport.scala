...
import cn.itcast.spark.utils.{JedisUtils, SparkUtils, StreamingUtils}
...
object DailyRealTimeOrderReport {
	...
	def saveToRedis(dataframe: DataFrame): Unit = {
		dataframe
			...
			.rdd.foreachPartition{iter =>
				// 1. 获取Jedis连接实例对象
				val jedis = JedisUtils.getJedisPoolInstance(
	...
	def main(args: Array[String]): Unit = {
		...
		val orderStreamDF: DataFrame = kafkaStreamDF
			...
			.as[String]
			// 过滤数据：通话状态为success
			.filter(record => null != record && record.trim.nonEmpty)
			// TODO: 提取字段信息
